...
module.exports = {
  ...
  async execute(params) {
    ...
    try {
      ...
      const { 
        ...
        AlignmentType, TableOfContents, StyleLevel,
      ...
      // 解析章节结构
      const { tableOfContents, sections, pageInfo } = this.parseContent(params.content);
      
      // 创建文档结构
      const docSections = [];
      
      // 1. 封面页
      const coverPage = this.createCoverPage(params, { Document, Paragraph, TextRun, AlignmentType, PageBreak });
      ...
      return {
        ...
        details: {
          ...
          structure: {
            ...
            pages: Math.ceil(sections.length / 2) + 3, // 估算页数
  ...
  parseContent(content) {
    ...
    return { tableOfContents, sections, pageInfo: { totalPages: currentPage } };
  },
  ...
  createCoverPage(params, { Paragraph, TextRun, AlignmentType, PageBreak }) {
    ...
    elements.push(
      new Paragraph({
        ...
        alignment: AlignmentType.CENTER,
        spacing: { before: 2880, after: 720 } // 上下间距
      })
    );
  ...
  createAbstractPage(params, { Paragraph, TextRun, HeadingLevel, PageBreak }) {
    ...
    elements.push(
      new Paragraph({
        ...
        spacing: { line: 360, after: 300 }, // 1.5倍行距
  ...
  createTableOfContents(tocEntries, { Paragraph, TextRun, HeadingLevel, Tab, PageBreak }) {
    ...
    tocEntries.forEach(entry => {
      const indent = (entry.level - 1) * 480; // 每级缩进2字符
  ...
  createContent(sections, { Paragraph, TextRun, HeadingLevel, AlignmentType }) {
    ...
    sections.forEach(section => {
      // 标题
      let headingLevel, fontSize, isBold;
